<?php
	...
	function wurfl_devices_buttons($object, $as_array=false) {
		...
		foreach($classes as $class) {
			$class_handler = xoops_getmodulehandler($class, 'wurfl');
			$criteria = new Criteria('`did`', $id);
			if ($class_handler->getCount($criteria)==0) {
				$ele[$class] = new XoopsFormButton('', $class, defined('_WURFL_FRM_BUTTON_CREATE_'.strtoupper($class))?constant('_WURFL_FRM_BUTTON_CREATE_'.strtoupper($class)):'_WURFL_FRM_BUTTON_CREATE_'.strtoupper($class));
				$ele[$class]->setExtra("onClick=\"window.open('".dirname($_SERVER['PHP_SELF']).'/'.$class.".php?op=".$class."&fct=create&did=".$id.$placement.'\',\'_self\')"');
			} else {
				$ele[$class] = new XoopsFormButton('', $class, defined('_WURFL_FRM_BUTTON_EDIT_'.strtoupper($class))?constant('_WURFL_FRM_BUTTON_EDIT_'.strtoupper($class)):'_WURFL_FRM_BUTTON_EDIT_'.strtoupper($class));
	...
	function wurfl_devices_form($object, $as_array=false) {
		...
		$ele['limit'] = new XoopsFormHidden('limit', !empty($_REQUEST['limit'])?intval($_REQUEST['limit']):30);
		$ele['start'] = new XoopsFormHidden('start', !empty($_REQUEST['start'])?intval($_REQUEST['start']):0);
	...
	function wurfl_form_general($object, $class, $new=false) {
		...
		$form = new XoopsThemeForm(defined('_WURFL_FRM_'.strtoupper($class).'_FORM')?constant('_WURFL_FRM_'.strtoupper($class).'_FORM'):($object->isNew()?_WURLF_FRM_CREATE:_WURLF_FRM_EDIT). ' '. str_replace('_UI', ' '._WURLF_FRM_UI, strtoupper($class)).' '._WURLF_FRM_WITHFORM, $class, $_SERVER['PHP_SELF'], 'post');

		$form->addElement(new XoopsFormHidden('op', $class));
		$form->addElement(new XoopsFormHidden('limit', !empty($_REQUEST['limit'])?intval($_REQUEST['limit']):30));
		$form->addElement(new XoopsFormHidden('start', !empty($_REQUEST['start'])?intval($_REQUEST['start']):0));
		...
		foreach($object->vars as $key => $definition) {
			switch ($key) {
				...
				default:
					...
					switch($field_type) {
						case 'yesno':
							$form->addElement(new XoopsFormRadioYN(defined('_WURFL_FRM_'.strtoupper($class).'_'.strtoupper($key))?constant('_WURFL_FRM_'.strtoupper($class).'_'.strtoupper($key)):ucfirst($class).' - '.ucfirst(str_ireplace('_', ' ', $key)), $key, $object->getVar($key)));
							break;
						case 'text':
							if ($definition['maxlength']>32)
								$form->addElement(new XoopsFormText(defined('_WURFL_FRM_'.strtoupper($class).'_'.strtoupper($key))?constant('_WURFL_FRM_'.strtoupper($class).'_'.strtoupper($key)):ucfirst($class).' - '.ucfirst(str_ireplace('_', ' ', $key)), $key, 45, $definition['maxlength'], $object->getVar($key)));
							elseif ($definition['data_type']==XOBJ_DTYPE_TXTBOX)
								$form->addElement(new XoopsFormText(defined('_WURFL_FRM_'.strtoupper($class).'_'.strtoupper($key))?constant('_WURFL_FRM_'.strtoupper($class).'_'.strtoupper($key)):ucfirst($class).' - '.ucfirst(str_ireplace('_', ' ', $key)), $key, 32, $definition['maxlength'], $object->getVar($key)));
							else
								$form->addElement(new XoopsFormText(defined('_WURFL_FRM_'.strtoupper($class).'_'.strtoupper($key))?constant('_WURFL_FRM_'.strtoupper($class).'_'.strtoupper($key)):ucfirst($class).' - '.ucfirst(str_ireplace('_', ' ', $key)), $key, 15, 20, $object->getVar($key)));
					}
			}
		}
