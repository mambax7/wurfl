	...
	function eventCoreHeaderStart($args)
	{
		...
		if (isset($user_agent)) {
			...
			if (is_object($GLOBALS['xoopsUser']))
				$out[microtime(true)] = array('useragent'=>$user_agent, 'theme' => $GLOBALS['xoopsConfig']['theme_set'], 'user' => '<a href="'.XOOPS_URL.'/userinfo.php?uid='.$GLOBALS['xoopsUser']->getVar('uid').'">'.$GLOBALS['xoopsUser']->getVar('uname').'</a>');
			else 
